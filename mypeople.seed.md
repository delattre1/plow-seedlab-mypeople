...
**8.5.4.1 NO-FLOOD + card-scoping (the fanout is a SIGNAL, not a firehose — proven 2026-06-22).**
- **NO-FLOOD.** The board→CTO event fanout MUST NOT ping the CTO (and through it the CEO) on every
  twitch of board churn. Four contracts, all env-tunable (`CTO_FANOUT_*`, nothing hard-coded):
  1. **Dedup — each change fires AT MOST ONCE.** Every comment is keyed by its comment-id and every
     state move by its `(task, from→to)` transition; a key already fanned NEVER fans again. On start
     the server **SEEDS its seen-set from the current board** so existing history is not replayed
     (no fanout storm on restart/re-poll).
  2. **Coalesce a same-card burst into ONE.** Multiple changes to the SAME card inside a short window
     collapse into a SINGLE `[cto]` event (suffixed "… +N more change(s) on this card") — five rapid
     comments yield ONE notification (`CTO_FANOUT_DEBOUNCE` settle, `CTO_FANOUT_COALESCE` per-card window).
  3. **Throttle — per-card cooldown + global min-gap.** After a card fans out, further changes to it
     are held + merged for `CTO_FANOUT_COALESCE`; no two `[cto]` events are emitted closer than a
     global floor (`CTO_FANOUT_MIN_GAP`). A FAILED delivery is **never silently dropped** — held +
     retried (bounded), abandoned only with a logged drop (a stall signal must not vanish).
  4. 🔴 **EXEMPT `{test}`/`[demo]`/`[proof]` fixtures — the SAME `_is_test` rule the Boss ping uses.**
     A change on a test/demo/proof card fans out NOTHING (reaches neither CTO nor CEO) — this is what
     lets the team validate on fixtures without flooding the CEO. (J43.)
- 🔴 **On-card comments stay CARD-SCOPED (CEO rule, 2026-06-22).** A CTO comment ON a card stays on
  THAT card's topic ONLY — never mixes in a decision/draft/status belonging to another card. The
  CTO's **cross-card decision queue** (all open drafts across every card) is a **DTU/chat concern**,
  surfaced over `send-whatsapp` one item at a time (each tagged with its card id) — NEVER pasted onto
  an unrelated card's thread.
...
- **WhatsApp drain** (`/todo/wa*`, Hermes last-hop): a host-specific notification bridge. **(The
  CTO proxy agent §8.5 now makes the Hermes inbound/outbound bridge an IN-SCOPE, gated component —
...
| `CTO_AGENT` | no | `<HOST_ID>/cto:CTO` | — | **The CTO proxy (§8.5).** Steps generate the CTO and set this in `queue.env` → `cto_fanout` is **on by default**. Unset disables every CTO path (no-op). |
| `CTO_TOKEN` | no | auto-generate | `grep` `queue.env` | The CTO's OWN machine credential (distinct from `QUEUE_SECRET`); the server derives `caller=CTO_AGENT` from it and binds the §8.5.3 hard rules to that authenticated identity. Steps generate if unset. |
| `CEO_WHATSAPP` | **no (host-specific)** | — | gitignored `queue.env` | CEO's WhatsApp digits — the ONLY `from` that can mint a delegation token (§8.5.3#2/§8.5.4). **NEVER hard-coded in the seed/commit** (privacy). Unset → delegated-create stays off; fanout + hard rules still gate. |
| `HERMES_SEND_URL` | **no (host-specific)** | — | gitignored `queue.env` | Live Hermes bridge **tailnet** send endpoint (§8.5.4/§8.5.7, Option A). Unset → `/cto/outbound` 501-stubs (no crash); **Verify points it at the DTU fixture** (§14) for the blind end-to-end. |
| `CTO_FANOUT_DEBOUNCE` / `CTO_FANOUT_COALESCE` / `CTO_FANOUT_MIN_GAP` | no | prod defaults (~5s / ~40s / global floor) | env | No-flood windows (§8.5.4.1): burst-settle debounce, per-card coalesce window, global min-gap between `[cto]` events. **Verify shrinks them to 1–2s** so J43 runs <5min. |
...
- **CTO proxy agent (§8.5)** — scaffold `run/cto/` (`CLAUDE.md` doctrine from the §8.5 intent +
  `plans/cto-claude.md`; `skills/send-whatsapp/`), the `todo-server` extensions (CTO event **fanout
  + no-flood** §8.5.4/§8.5.4.1; the two SERVER-ENFORCED hard rules §8.5.3; the one-shot delegation
  token mint/burn; `/cto/inbound` **with machine auth before reading `from`** + `/cto/outbound`
  **argv, no shell**), a **durable CTO supervisor** (§5.3-style — respawns `cto:CTO` independent of
  the Boss, since it is a top-level always-on overnight agent), and the two thin functions wired to
  the host's **EXISTING** Hermes (8.5.4). NO new bridge; NO CTO logic in Hermes.
...
7.5. **CTO proxy agent (§8.5).** Scaffold `$INSTALL_DIR/run/cto/` (`CLAUDE.md` from the §8.5 intent +
   `plans/cto-claude.md`; `skills/send-whatsapp/`); pre-trust the cwd (§5.5c). Into `queue.env` write
   `CTO_AGENT` (→ `cto_fanout` ON by default) + auto-gen `CTO_TOKEN` (the CTO's machine credential,
   distinct from `QUEUE_SECRET`); and — **ONLY from the operator's env if present, NEVER a literal in
   this seed** — `CEO_WHATSAPP` (PII) + `HERMES_SEND_URL` (the Hermes host's **tailnet** `:3000/send`,
   never `127.0.0.1`/LAN). The CTO still installs + passes J39–J44 with both unset (the DTU §14 covers
   the blind E2E). **Spawn the CTO** (`mp spawn <host>/cto:CTO --boss <host>/main:Boss --cwd
   $INSTALL_DIR/run/cto`) — NOT `--master` — and **start its OWN durable supervisor** (respawns it
   independent of the Boss). Wire its two thin functions to the **EXISTING** Hermes over the tailnet
   (Option A, §8.5.7); if `HERMES_SEND_URL`/`hermes` is absent the endpoints stub (501) — do NOT build
   a new bridge, use localhost/LAN, or block install on Hermes.
...
### 14.1 CTO Verify fixture — the DTU (fake-Hermes) for BLIND proof (§8.5; gates J43/J44)

A blind container cannot pair WhatsApp, so `## Verify` stands up a **DTU** — a tiny fake-Hermes on
the **SAME seam §8.5.4 defines**, so the CTO brain is UNCHANGED and the whole draft→approve→post loop
is provable offline. **OUTBOUND:** `HERMES_SEND_URL`→DTU `/send`, which RECORDS the message as a
CEO-facing draft. **INBOUND:** Verify POSTs DTU `/inject`; the DTU then POSTs an **authenticated**
`/cto/inbound {from:<CEO_WHATSAPP>, text}` (with `X-Queue-Secret`) — exactly the call the real Hermes
webhook makes. Generate it as a Verify-only fixture (reference shape):

```sh
# DTU = Verify-only fake-Hermes bridge. Replaces the real WhatsApp transport; same seam as §8.5.4.
cat > "$INSTALL_DIR/run/cto/dtu.py" <<'DTU'
import json, os, urllib.request
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer
SECRET = os.environ["QUEUE_SECRET"]
CEO    = os.environ.get("CEO_WHATSAPP", "15550000000")
BOARD  = "http://127.0.0.1:%s" % os.environ.get("TODO_PORT", "9933")
OUT    = []                                              # CEO-facing drafts the CTO has sent
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        return self._j(200, {"out": OUT}) if self.path == "/thread" else self._j(404, {})
    def do_POST(self):
        b = self._body(); p = self.path.split("?", 1)[0]
        if p in ("/send", "/cto/outbound"):             # OUTBOUND: CTO -> CEO (record the draft)
            OUT.append(str(b.get("message") or b.get("text") or "")); return self._j(200, {"ok": True})
        if p == "/inject":                              # Verify "CEO replies" -> AUTHED inbound hop
            data = json.dumps({"from": CEO, "text": str(b.get("text") or "")}).encode()
            r = urllib.request.Request(BOARD + "/cto/inbound", data=data,
                headers={"X-Queue-Secret": SECRET, "Content-Type": "application/json"}, method="POST")
            try: urllib.request.urlopen(r, timeout=15)
            except Exception as e: return self._j(502, {"error": str(e)})
            return self._j(200, {"ok": True})
        return self._j(404, {})
ThreadingHTTPServer(("127.0.0.1", int(os.environ.get("DTU_PORT", "9934"))), H).serve_forever()
DTU
DTU_PORT=9934 python3 "$INSTALL_DIR/run/cto/dtu.py" &   # start the fake bridge
export HERMES_SEND_URL="http://127.0.0.1:9934/send"     # CTO outbound -> DTU (NOT real Hermes)
# Verify also sets CTO_FANOUT_DEBOUNCE=1 CTO_FANOUT_COALESCE=2 CTO_FANOUT_MIN_GAP=1 (coalesce flushes
# in seconds) + CTO_TOKEN + CTO_AGENT + CEO_WHATSAPP for the authed paths.
```

**J44 E2E run:** (1) trigger a board event (`POST /todo/comment` on a non-test card, by an engineer)
→ `cto_fanout` fires; (2) within the draft window `GET DTU/thread` shows ONE `out[]` entry from the
CTO bearing the **APPROVE/EDIT/REJECT** menu; (3) `POST DTU/inject {"text":"approve"}` → DTU posts the
authed `/cto/inbound` → the approval reaches the CTO queue; (4) assert the card now carries a comment
`by=<host>/cto:CTO`, its `state` is **NOT** `done`, and **no** comment on it claims `by:"CEO"`.
...
39. **CTO proxy agent is alive in its OWN folder + profile (§8.5.1, CEO 2026-06-21).** After install,
    `mp status` / `GET /agents` shows **`<host>/cto:CTO [alive]`** with `boss_id=<host>/main:Boss`
    (NOT a master). Its tmux window is `mc-cto:CTO` (cwd `$INSTALL_DIR/run/cto`), `run/cto/CLAUDE.md`
    + `run/cto/skills/send-whatsapp/` exist, and its **durable onboarding summary carries ≥2** of
    {`cto`,`ceo-equivalent`,`approve`,`whatsapp`,`never-done`} (like J2c). A CTO that is
    a master, has no folder/skill, or a 0-keyword summary = FAIL.
40. **CTO NEVER posts as the CEO — IDENTITY BOUND TO AUTH, not the body (§8.5.1/§8.5.3, CEO
    2026-06-21 + knightwatch).** A request **authenticated as the CTO** (header `CTO_TOKEN`) whose body
    claims `by`/`actor` = `"CEO"` (or anything ≠ `CTO_AGENT`) — on `/todo/comment`, `/todo/status`, or
    `/todo/update` — is **REJECTED `{ok:false, error:"cto_cannot_spoof"}`** BEFORE any other check; the
    board is unchanged. A legit CTO write (`by=<host>/cto:CTO`) posts fine. 🔴 The server must derive
    the caller from `CTO_TOKEN`, NEVER trust body `by`/`actor` — a CTO-authed caller that lands ANY
    write as `by:"CEO"` = FAIL (forgeable-identity bypass).
41. **CTO can NEVER mark done — CEO-only, bound to the authenticated caller (§8.5.3 #1, CEO
    2026-06-21 + knightwatch).** With a task on the board, EACH of these from an **authenticated CTO
    caller** (`CTO_TOKEN`) returns **`{ok:false, error:"cto_cannot_done"}`** with `state` **unchanged**:
    `POST /todo/status {state:"done"}`; `POST /todo/update {op:set,state:"done"}`; `set{done:true}`;
    `set{workToDone:true}`. 🔴 **SPOOF CANNOT BYPASS:** the same calls from the CTO caller but with
    body `by:"CEO"`/`actor:"CEO"` do NOT succeed — they return `cto_cannot_spoof` (the done block is
    keyed off the authenticated identity, not the body). The same calls from the **CEO** (authed
    WITHOUT `CTO_TOKEN`) succeed (proving it is CTO-specific, not a global lock). Any CTO done-transition
    that lands — directly or by claiming `by:"CEO"` — = FAIL.
42. **CTO create-task is gated on a one-shot CEO token, minted ONLY by AUTHENTICATED inbound and
    delivered VIA THE QUEUE (§8.5.3 #2 / §8.5.4, CEO 2026-06-21 + knightwatch).** (a) `POST
    /todo/update {op:add}` from an **authenticated CTO caller** (`CTO_TOKEN`) with NO token →
    **`{ok:false, error:"cto_cannot_create"}`**, board unchanged; the SAME `add` claiming body
    `by:"CEO"` to dodge the gate → **`cto_cannot_spoof`** (no task created). (b) Feed an **authenticated** inbound CEO
    WhatsApp "CTO, create <X>" to `POST /cto/inbound` (header `X-Queue-Secret`,
    `{from:<CEO_WHATSAPP>, text}`) → a one-shot token is minted. 🔴 **The token must arrive in the
    CTO QUEUE EVENT (the `[cto] … token=<minted>` that the server `mp send`s to `CTO_AGENT`), NOT be
    read from the webhook RESPONSE** (the response goes to Hermes, not the CTO). Assert the queued
    `[cto]` event carries the token; the CTO's `add` presenting **that queued token** as `{op:"add",
    text, actor:<CTO>, token:<minted>}` **succeeds** (task created). A token present only in the
    webhook response but absent from the queue event = FAIL (the real CTO would never get it). (c)
    🔴 **SPOOF REJECTED:** the SAME POST WITHOUT `X-Queue-Secret` (an unauthenticated caller claiming
    `from=<CEO_WHATSAPP>`) → **401, mints NOTHING**; a subsequent CTO `add` still returns
    `cto_cannot_create`. (d) **Reuse fails:** a minted token on a second `add` → rejected (burned).
    (e) **Expiry fails:** mint a token, advance past its TTL (drive a short expiry), then `add` with
    it → **`cto_cannot_create`**. (f) A token for a NON-CEO `from` (even authenticated) is never
    minted. An ungated CTO create, a token only in the webhook response, a reusable/expired token, or
    a token minted from an unauthenticated/spoofed inbound = FAIL.
43. **cto_fanout + no-flood (the proactive-notify; transport-free — §8.5.4/§8.5.4.1, CEO 2026-06-22).**
    With `CTO_AGENT` set: `POST` a non-test `/todo/comment` → within ~3s the **CTO queue holds EXACTLY
    ONE `[cto]` event** for that card (assert via the `[cto] …` delivery into `mc-cto:CTO`/the queue —
    NO transport needed); `POST /todo/status {state:"review"}` → a `[cto]` "blocked-on-CEO" event fires.
    **NO-FLOOD:** 5 rapid comments on ONE card → **EXACTLY ONE** coalesced `[cto]` event (suffixed
    "+N more"), not 5; a comment whose `by` is the CTO itself → **ZERO** (no self-loop); a comment on a
    `[test]`/`[demo]`/`[proof]` card → **ZERO**. The §6 Boss ping (J3/J32) still passes unchanged.
    (Verify sets `CTO_FANOUT_DEBOUNCE/COALESCE/MIN_GAP` small so coalescing flushes in seconds.) A
    missing fanout on a real event, 5-becomes-5 (no coalesce), a CTO self-loop, a `[test]` fanout, or a
    regressed Boss ping = FAIL.
44. **Hermes bridge = two thin secure functions + E2E draft→approve→post, PROVEN BLIND via the DTU
    (§8.5.4/§8.5.7, CEO 2026-06-21 + 06-22).** (a) `HERMES_SEND_URL` unset → `/cto/outbound` returns
    **501** (stub, no crash). (b) `/cto/inbound` requires `X-Queue-Secret` → **401 BEFORE `from` is
    read**; outbound invokes the transport **argv-only** (`shell=False`) — a `text` with shell
    metacharacters (`; touch /tmp/pwned`, `$(…)`, backticks) executes NOTHING (assert no `/tmp/pwned`).
    (c) the generated `todo-server` does **NOT hard-code `127.0.0.1`/`localhost`/`192.168.x`** as the
    Hermes endpoint — it reads `HERMES_SEND_URL` (tailnet at runtime). (d) **E2E via the DTU fixture
    (§14):** point `HERMES_SEND_URL`→DTU; a board event → the CTO **drafts** → `GET DTU/thread` shows
    ONE draft bearing the **APPROVE/EDIT/REJECT** menu → `POST DTU/inject {"text":"approve"}` (DTU
    relays an AUTHED `/cto/inbound`) → the CTO **posts the comment as `by=<host>/cto:CTO`**; the card
...
| F25 | CTO proxy + proactive-notify | event-driven in-server `cto_fanout` (gated on `CTO_AGENT`) + no-flood (dedup/coalesce/throttle/`[test]`-exempt) + card-scoped; draft→approve→post-as-cto over Hermes, proven blind via the DTU | J39–J44 |
